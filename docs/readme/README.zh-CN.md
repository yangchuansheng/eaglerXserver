...
1. 打开游戏入口，或使用服主分享的管理面 Overview 快速加入链接。多人游戏列表已默认包含本服务器：客户端根据页面地址推导 `ws://` 或 `wss://` 游戏地址，因此自定义端口和 HTTPS 游戏入口无需额外配置；快速加入链接会直接连接。如需加入其他服务器，可在多人游戏中添加其 `ws://HOST:5200/` 地址。
