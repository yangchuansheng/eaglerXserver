...
1. 게임 URL이나 서버 운영자가 관리자 패널의 Overview에서 공유한 빠른 접속 링크를 여세요. 멀티플레이 목록에는 이 서버가 이미 표시됩니다. 클라이언트가 페이지 URL에서 `ws://` 또는 `wss://` 주소를 유도하므로 사용자 지정 포트와 HTTPS 게임 접속에도 추가 설정이 필요 없습니다. 빠른 접속 링크는 바로 연결됩니다. 다른 서버에 접속하려면 멀티플레이에서 해당 `ws://HOST:5200/` 주소를 추가하세요.
