...
```bash
docker pull ghcr.io/yangchuansheng/eaglerx1.8server:2.2.4
...
管理面支持 English 与简体中文，页眉语言选择会按浏览器 origin 保存在 `localStorage`。
...
```bash
gh workflow run release.yml -f release_tag=v2.2.4
```
